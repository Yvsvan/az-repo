...
class _AccountList(ctk.CTkScrollableFrame):
    """Multi-select checkbox list of loaded SAT credentials."""
    ...
    def refresh(self, credentials: list[SatCredential]) -> None:
        ...
        for i, c in enumerate(credentials):
            ...
            ctk.CTkCheckBox(
                ...
                fg_color=TEAL,
            ).grid(row=i, column=0, sticky="w", pady=1, padx=4)
...
class SatTab(ctk.CTkFrame):
    ...
    def _build_accounts(self) -> None:
        ...
        ctk.CTkButton(
            ...
            command=lambda: self._account_list.select_none(),
        ...
        ctk.CTkButton(
            ...
            border_color=BORDER_DEFAULT,
            hover_color=BG_SURFACE,
            command=lambda: self._account_list.select_all(),
    ...
    def _remove_account(self) -> None:
        ...
        indices = self._account_list.selected_indices

        if not indices:
            messagebox.showinfo(
                "Seleccionar cuenta",
                "Marca al menos una cuenta en la lista para eliminarla.",
            )

            return

        creds_to_remove = [self._credentials[i] for i in indices]

        names = "\n".join(f"  • {c.display_name}" for c in creds_to_remove)

        if not messagebox.askyesno(
            "Confirmar eliminación",
            f"¿Eliminar {len(creds_to_remove)} cuenta(s)?\n{names}\n\nSe borrarán las contraseñas guardadas.",
        ):
            return

        from bank_parser.sat_downloader.auth import delete_password

        for cred in creds_to_remove:
            delete_password(cred.rfc)

            self._credentials = [c for c in self._credentials if c.rfc != cred.rfc]

            self._s.sat.credentials = [c for c in self._s.sat.credentials if c.rfc != cred.rfc]

            self._log.info(f"Cuenta eliminada: {cred.display_name}")
    ...
    def _reload_saved_credentials(self) -> None:
        ...
        for ref in self._s.sat.credentials:
            # Evitar duplicados si settings.json tiene entradas repetidas
            if any(c.rfc == ref.rfc for c in self._credentials):
                self._log.warn(f"RFC duplicado en configuración, omitido: {ref.rfc}")
                continue
    ...
    def _start(self) -> None:
        ...
        selected_indices = self._account_list.selected_indices

        if not selected_indices:
            messagebox.showwarning(
                "Sin cuentas seleccionadas",
                "Marca al menos una cuenta en la lista para descargar.",
            )

            return

        selected_creds = [self._credentials[i] for i in selected_indices]
        ...
        self._log.info(
            f"Iniciando descarga para {len(selected_creds)} cuenta(s) | "
        ...
        threading.Thread(
            target=self._worker,
            args=(selected_creds, fecha_ini, fecha_fin, tipo, output, poll),
            daemon=True,
        ).start()
    ...
    def _worker(self, creds, fecha_ini, fecha_fin, tipo, output, poll) -> None:
        ...
        try:
            results = run_download_session(
                ...
                sat_api_url=self._s.sat.sat_api_url,
                sat_descarga_url=self._s.sat.sat_descarga_url,
            )
